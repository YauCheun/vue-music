<template>
  <div class="detaillist-contanier"  
    ...
    <div class="top-wrap">
      ...
      <div class="info-wrap">
        <!-- 名字 -->
        <p class="title"><span>专辑</span>{{ playlist.name }}</p>
        <div class="author-wrap">
          <!-- 名字 -->
          <span class="name" v-if="playlist.creator !== undefined">{{ playlist.creator.nickname }}</span>
          <!-- 时间 -->
          <span class="time">{{ playlist.publishTime | dateFormat }}</span>
        </div>
        <div class="btn-box">          
          <div class="play-wrap">
...
export default {
  ...
  methods:{
    ...
    async getDetailList(id){
      try {
        let res = await api.getAlbumContent({id})
        console.log(res)
        this.playlist=res.data.album
        this.nickname = res.data.album.name
    ...
    async getPlaylistComments() {
      try {
        const res = await api.getAlbumComments({
          id: this.id,
          limit: this.pageSize,
